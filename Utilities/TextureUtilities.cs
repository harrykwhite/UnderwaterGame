...
namespace UnderwaterGame.Utilities
{
    ...
    {
        public static Vector2 TextureSize(Texture2D texture) => new Vector2(texture.Width, texture.Height);
        ...
        public static Texture2D CreateOutlined(Texture2D texture, Color color, bool resize = true)
        {
            ...
            for (int y = 0; y < texture.Height; y++)
            {
                for (int x = 0; x < texture.Width; x++)
                {
                    ...
                    if (x > 0) { left = textureDataTwoDimensional[x - 1, y].A > 0; }
                    if (y > 0) { top = textureDataTwoDimensional[x, y - 1].A > 0; }

                    if (x < texture.Width - 1) { right = textureDataTwoDimensional[x + 1, y].A > 0; }
                    if (y < texture.Height - 1) { bottom = textureDataTwoDimensional[x, y + 1].A > 0; }
